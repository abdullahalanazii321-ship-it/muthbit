...
## المكدّس
- `apps/api` — Node 20 + Express + Knex + PostgreSQL 16. **جاهز ومختبر (٦٧ فحصاً ناجحاً). لا تعدّل أي ملف تحته إلا بطلب صريح مني.**
- `apps/web` — React + Vite + Tailwind. **غير موجود بعد.** أول مهمة واجهة تنشئه.
...
npm run api:test        # ٦٧ فحصاً — يجب أن تمر كلها
```
